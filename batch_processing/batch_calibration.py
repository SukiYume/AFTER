...
def collect_calibration_groups(
    root_dir,
    cal_root,
    sources,
    cal_npz,
        ...
        for date in dates:
            ...
            for beam, h5_list in sorted(beam_groups.items()):
                ...
                for sibling_date in dates:
                    if (
                        sibling_date == date
                        or sibling_date.split("_", 1)[0] != date_base
                    ):
                        continue
                    ...
                    if (
                        sibling_path is not None
                        and sibling_path not in cal_fits_candidates
                    ):
...
def process_group(group):
    ...
    for cal_fits_path in candidates:
        ...
        details = "; ".join(
            f"{os.path.basename(path)}: {error}" for path, error in calibration_errors
        )
        raise ValueError(
...
def parse_args():
    ...
    parser.add_argument(
        "--rfi-down-freq", type=int, default=None, help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--rfi-fft",
...
if __name__ == "__main__":
    ...
    if args.output_time_samples is not None and args.time_crop_samples is not None:
        raise SystemExit(
            "--output-time-samples and --time-crop-samples are mutually exclusive"
        )
    batch_calibrate(
        args.root_dir,
        args.cal_root,
