...
def _fit_gaussian(x, y, snr_min=3.0):
    ...
    bounds_lo = [
        0,
        xf[0] - 0.5 * dx,
        np.abs(xf[1] - xf[0]) * 0.1,
        med - 3 * max(mad, abs(amp0)),
    ]
    bounds_hi = [amp0 * 5, xf[-1] + 0.5 * dx, dx, med + 3 * max(mad, abs(amp0))]
...
def _empty_props(file_mjd, freq, fs, fe, nchan):
    ...
    return {
        ...
        "bandwidth_gauss": np.nan,
        "bandwidth_gauss_err": np.nan,
    }


def calc_burst_properties(
    stokes_I,
    freq,
    time_reso,
    file_mjd,
    ...
    if gain is not None and gain_err is not None and np.any(freq_index):
        ...
        if mean_gain > 0 and (mean_gain + mean_gain_err) > 0:
            sys_factor = mean_gain * (
                1.0 / mean_gain - 1.0 / (mean_gain + mean_gain_err)
            )
        else:
            sys_factor = 0.0
    ...
    if flux_peak > 0 and fluence > 0:
        ...
        width_err = width * np.sqrt(
            (total_flu_err / fluence) ** 2 + (total_flux_err / flux_peak) ** 2
        )
    else:
        width_err = 0.0
