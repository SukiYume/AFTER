...
def extract_segment(data_path, file_list, info, start_sample, total_length):
    ...
    for fi in range(first_file_idx, last_file_idx + 1):
        ...
        if available:
            segment[cursor : cursor + available] = fdata[local_start:available_end]
        if available < n_copy:
            print(
...
def cut_one_burst(
    ...
    try:
        with h5py.File(temp_path, "w") as f:
            f.create_dataset(
                "data", data=data_dedispersed, compression="gzip", compression_opts=4
            )
