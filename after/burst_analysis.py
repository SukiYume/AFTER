...
def _empty_pol_result(freq, status, reason):
    ...
    center_freq = float(np.mean(finite_freq)) if finite_freq.size else np.nan
    return {
        "rm": np.nan,
        "rm_err": np.nan,
...
def _downsample_channel_mask(mask, factor, target_size):
    ...
    if factor <= 1:
        ...
        result = np.asarray(mask[:n_keep].reshape(-1, factor).any(axis=1), dtype=bool)
        result = result[:target_size]
    if result.size < target_size:
        result = np.pad(result, (0, target_size - result.size), constant_values=False)
    return result


def _apply_extra_downsample(
    iquv, freq, gain, gain_err, burst_regions, time_reso, extra_dt, extra_df
):
...
def plot_dynamic_spectrum(stokes_I, freq, time_reso, burst_regions, save_path):
    ...
    ax1.imshow(
        ...
        vmin   = vmin,
        vmax   = vmax,
        extent = (float(time_ms[0]), float(time_ms[-1]), float(freq[0]), float(freq[-1])),
    )
...
def _select_strong_time_samples(
    stokes_I, freq_index, noise_mask, burst_region, peak_fraction=0.5, min_snr=5.0
):
    ...
    smooth        = np.convolve(net_filled, np.array([0.25, 0.5, 0.25]), mode="same")
...
def plot_rm_selection(
    stokes_I, freq, time_reso, burst_region, rm_time_mask, rm_freq_index, save_path
):
    ...
    ax0.axvspan(
        burst_region["time_start"] * time_reso * 1e3,
...
def analyze_one_file(
    ...
    with h5py.File(cal_h5_path, "r") as f:
        ...
        gain = (
            np.asarray(gain_dataset[...])
            if isinstance(gain_dataset, h5py.Dataset)
            else None
        )
        gain_err = (
            np.asarray(gain_err_dataset[...])
            if isinstance(gain_err_dataset, h5py.Dataset)
            else None
        )
        stored_cal_channel = (
            np.asarray(cal_channel_dataset[...], dtype=bool)
            if isinstance(cal_channel_dataset, h5py.Dataset)
            else None
        )
        stored_burst_channel = (
            np.asarray(burst_channel_dataset[...], dtype=bool)
            if isinstance(burst_channel_dataset, h5py.Dataset)
            else None
        )
    ...
    burst_regions = [
        r
        for r in burst_regions
        if r["time_end"] > r["time_start"] and r["freq_end"] > r["freq_start"]
    ]
    if len(burst_regions) < n_before:
        print(
            f"  [{basename}] 下采样后 {n_before - len(burst_regions)} 个 "
            f"爆发区间退化为空, 跳过"
        )
    if not burst_regions:
        return []
    ...
    if strongest_burst_only and len(indexed_regions) > 1:
        ...
        for original_idx, candidate in indexed_regions:
            ...
            scored.append((candidate_info["peak_snr"], original_idx, candidate))
        _, keep_idx, keep_region = max(scored, key=lambda item: item[0])
        indexed_regions          = [(keep_idx, keep_region)]
        print(
            f"  [{basename}] strongest-burst-only: 保留爆发 {keep_idx}, "
    ...
    for bi, region in indexed_regions:
        ...
        row = {
            ...
            "rm_time_start_ms": (
                float(rm_indices[0] * time_reso * 1e3) if rm_indices.size else np.nan
            ),
            "rm_time_end_ms": (
                float((rm_indices[-1] + 1) * time_reso * 1e3)
                if rm_indices.size
                else np.nan
            ),
            ...
            "rm_freq_low_mhz": (
                float(np.min(freq[rm_freq_index])) if np.any(rm_freq_index) else np.nan
            ),
            "rm_freq_high_mhz": (
                float(np.max(freq[rm_freq_index])) if np.any(rm_freq_index) else np.nan
            ),
...
def analyze_all(
    ...
    h5_files = sorted(
        glob.glob(os.path.join(cal_dir, "**", "*_cal.h5"), recursive=True)
    )
    if not h5_files:
    ...
    for h5_path in h5_files:
        file_results = analyze_one_file(
            ...
            rm_freq_min          = rm_freq_min,
            rm_freq_max          = rm_freq_max,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir", default=None, help="输出目录；默认写到 <cal-dir>/analysis"
    )
    parser.add_argument(
        "--rfi-fft", default=RFI_FFT, action="store_true", help="RFI 改用 FFT 法"
    )
    rfi_mode = parser.add_mutually_exclusive_group()
    rfi_mode.add_argument(
        "--rfi-channel-only",
        dest    = "rfi_channel_only",
        action  = "store_true",
        default = True,
    ...
    parser.add_argument(
        "--rfi-channel-sigma",
        default = RFI_CHANNEL_SIGMA,
        type    = float,
        help    = "局部鲁棒通道 RFI 阈值 (默认 6)",
    )
    parser.add_argument(
        "--rfi-channel-window",
        default = RFI_CHANNEL_WINDOW,
        type    = int,
        help    = "局部通道统计的频率窗口 (默认 31)",
    )
    parser.add_argument(
        "--rfi-channel-grow",
        default = RFI_CHANNEL_GROW,
        type    = int,
        help    = "坏通道向两侧扩展的频道数 (默认 1)",
    )
    ...
    parser.add_argument(
        ...
        default = RM_MIN,
        type    = float,
        help    = "RM 下限；网格步长由有效 lambda^2 覆盖自动计算",
    )
    parser.add_argument(
        "--rm-max",
        default = RM_MAX,
        type    = float,
        help    = "RM 上限；网格步长由有效 lambda^2 覆盖自动计算",
    )
    parser.add_argument(
        "--rm-peak-fraction",
        default = RM_PEAK_FRACTION,
        type    = float,
        help    = "RM 时间门的主峰分数阈值 (默认 0.5)",
    )
    parser.add_argument(
        "--rm-min-time-snr",
        default = RM_MIN_TIME_SNR,
        type    = float,
        help    = "RM 时间门的最低 I 轮廓 S/N (默认 5)",
    )
    parser.add_argument(
        "--rm-freq-min",
        default = None,
    ...
    df = analyze_all(
        ...
        rm_freq_min          = args.rm_freq_min,
        rm_freq_max          = args.rm_freq_max,
