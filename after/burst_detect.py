...
import seaborn  # noqa: F401  # 仅导入以注册颜色映射，不直接调用模块成员
import torch
...
class _ReviewState(TypedDict):
    """Mutable state shared by Matplotlib callbacks."""
...
def load_yolo_model(model_path, model_name="yolo11n"):
    ...
    model_with_args = cast(Any, model)
    if getattr(model_with_args, "args", None) is None:
        model_with_args.args = get_cfg()
    model_with_args.args.box = 7.5
    model_with_args.args.cls = 0.5
    model_with_args.args.dfl = 1.5
...
def prepare_calibration_display(
    stokes_I, freq, time_reso, time_factor=1, freq_factor=1
):
    ...
    if time_factor > 1:
        ...
        plot_I = np.nanmean(
            plot_I[: nt * time_factor].reshape(nt, time_factor, nchan),
            axis=1,
        )
        nsamp = nt
    ...
    if freq_factor > 1:
        ...
        plot_I = np.nanmean(
            plot_I[:, : nc * freq_factor].reshape(nsamp, nc, freq_factor),
            axis=2,
        )
        plot_freq = plot_freq[: nc * freq_factor].reshape(nc, freq_factor).mean(axis=1)
...
def prepare_image_tiles(stokes_I, target_size=512, time_factor=None, freq_factor=None):
    ...
    if freq_factor > 1:
        ...
        data = (
            data[:, :nc_keep]
            .reshape(nsamp, nc_keep // freq_factor, freq_factor)
            .mean(axis=2)
        )
...
def filter_inference_boxes(
    scores, boxes, max_horizontal_aspect=DEFAULT_MAX_HORIZONTAL_ASPECT
):
...
def predict_single(
    model, img_float32, conf=0.25, max_horizontal_aspect=DEFAULT_MAX_HORIZONTAL_ASPECT
):
    ...
    if score_np is None or box_np is None:
        return None, None
...
def add_region_patches(
    ...
    for r in burst_regions:
        ...
        rect, span = _draw_burst_box(
            ax_profile, ax_spec, t0, t1, f0, f1, edge=edge, face=face, lw=linewidth
        )
        if label_conf:
            ax_spec.text(
                t0,
...
def review_interactive(
    ...
    state: _ReviewState = {
        # 三种结束指令依次表示接受当前框、确认无爆发、退出整批处理。
        "command": None,
        "showing_model": showing_model,
        # 每项同时保存原始坐标、动态谱矩形和时间轮廓阴影，便于成组撤销。
        "user_boxes": [],
    }
    ...
    def on_select(eclick, erelease):
        ...
        rect, span = _draw_burst_box(
            ax_prof, ax_spec, x0, x1, y0, y1, edge="yellow", face="gold", lw=1.5
        )
    ...
    def on_key(event):
        """处理复核窗口的快捷键，并结束当前 Matplotlib 事件循环。

        Enter 接受当前区域，X 明确记录为空，Q/Escape 请求退出整批流程。
        回调只写入 ``state``；最终如何保存由外层函数统一决定。
        """
    ...
    if showing_model:
        print(
            "  图窗内按 Enter 接受模型框, 按 x 标为无爆发, 鼠标左键拖拽即重画 (右键撤销), q/Esc 退出。"
        )
    else:
        print(
            "  在图窗中拖拽鼠标左键画爆发框, 按 x 标为无爆发, 右键撤销最近一框, 画完按 Enter, q/Esc 退出。"
        )
...
def detect_one_file(
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        if not isinstance(data_dataset, h5py.Dataset):
            raise TypeError(f"H5 'data' is not a dataset: {h5_path}")
    ...
    if mode == "manual":
        ...
        burst_regions = boxes_to_regions_tiled(
            tile_results, time_factor, freq_factor, nsamp, nchan
        )
    ...
    if plot_dir is not None:
        ...
        plot_path = os.path.join(plot_dir, os.path.splitext(fname)[0] + "_det.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-path", default=str(DEFAULT_DETECTOR_MODEL), help="YOLO 权重文件路径"
    )
    parser.add_argument("--model-name", default="yolo11n", help="YOLO 模型配置名")
    parser.add_argument(
        "--output-dir", default=None, help="输出目录；默认写到 <cal-dir>/detections"
    )
    ...
    parser.add_argument(
        "--rfi-fft", action="store_true", help="检测确认后写入 H5 的 RFI 改用 FFT 法"
    )
    ...
    model = (
        None
        if args.mode == "manual"
        else load_yolo_model(args.model_path, args.model_name)
    )

    # 查找定标 h5 文件
    h5_files = sorted(
        glob.glob(os.path.join(args.cal_dir, "**", "*_cal.h5"), recursive=True)
    )
    ...
    if quit_requested:
        ...
        print(f"\n[退出] 当前进度已保存: {det_path}")
        print(
            f"已处理 {len(detections)}/{len(h5_files)} 个文件，"
            f"其中 {n_with} 个检测到爆发"
        )
    else:
