...
def draw_preview(img, bboxes, pred_conf, pred_boxes, sample_idx):
    ...
    if isinstance(bboxes, np.ndarray) and bboxes.ndim == 2 and len(bboxes) > 0:
        for x, y, w, h in bboxes.astype(np.int32):
            ...
            cv2.drawMarker(image_vis, (int(x + w / 2), int(y + h / 2)),
    ...
    if pred_boxes is not None:
        for bi, (cx, cy, w, h) in enumerate(pred_boxes.astype(np.int32)):
            ...
            cv2.drawMarker(image_vis, (int(cx), int(cy)),
                           (0, 0, 255), cv2.MARKER_CROSS, 6, 1)
...
def main(args):
    ...
    _, val_df = get_train_val(args.data_path, val_fraction=args.val_fraction, seed=args.seed)
    ...
    try:
        for sample_idx in preview_indices:
            ...
            pred_conf, pred_boxes = predict_single(
                model, img, conf=args.conf, iou_threshold=args.iou_threshold,
            )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='AFTER burst detector 推理 (直接读 H5 验证集)')
