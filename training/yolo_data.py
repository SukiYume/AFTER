...
def get_train_val(
    ...
    seed=42,
    empty_repeat=1,
    single_burst_repeat=3,
    multi_burst_repeat=6,
):
    ...
    for group in groups:
        ...
        if not val_groups or abs(next_count - target_val_count) < abs(
            val_count - target_val_count
        ):
...
class H5YOLODataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataframe, imgsz=IMAGE_SIZE, val=False, mosaic_prob=0.5):
        if int(imgsz) != IMAGE_SIZE:
            raise ValueError(
                f"AFTER burst detector uses a fixed image size of {IMAGE_SIZE}"
            )
    ...
    def __getitem__(self, idx):
        ...
        img_tensor = (
            torch.from_numpy(img).float().unsqueeze(0).expand(3, -1, -1).contiguous()
        )
    ...
    def _augment(self, img, bboxes):
        ...
        return augmented["image"], np.array(bboxes_aug) if len(
            bboxes_aug
        ) else np.array([])
    ...
    def _build_train_transform(self):
        ...
        return A.Compose(
            ...
            bbox_params=A.BboxParams(
                format="coco", min_visibility=0.1, label_fields=[]
            ),
        )
    ...
    def _format_targets(self, bboxes):
        ...
        n = len(cls_list)
        cls = (
            torch.tensor(cls_list, dtype=torch.float32).reshape(n, 1)
            if n
            else torch.zeros((0, 1))
        )
        boxes = (
            torch.tensor(bbox_list, dtype=torch.float32) if n else torch.zeros((0, 4))
        )
        return cls, boxes
