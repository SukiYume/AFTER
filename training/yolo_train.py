...
def select_device(device_arg):
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            f"CUDA device {device_arg} was requested, but CUDA is unavailable"
        )
    try:
...
def main(args):
    ...
    log_dir = (
        os.path.abspath(args.log_dir) if args.log_dir else default_log_dir(args.model)
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        logs.append(
            build_log_entry(epoch, current_lr, train_m, val_m, metrics, metrics_raw)
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="AFTER burst detector YOLO H5 training"
    )
    ...
    # hardware
    parser.add_argument(
        "--device", type=str, default="0", help="单张 CUDA GPU 编号（如 0）或 cpu"
    )
    ...
    parser.add_argument(
        "--nms-iou-thr", type=float, default=0.65, help="同图内 NMS IoU 阈值"
    )
    parser.add_argument(
        "--match-iou-thr",
        type    = float,
