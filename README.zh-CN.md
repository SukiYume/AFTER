...
面向 FAST FRB burst 的搜索后处理流程：从 TOA 列表到定标测量、诊断图、结果表和观测面板。
...
**AFTER** 是一套面向 FAST fast radio burst (FRB) 观测的搜索后处理流程。上游搜索流程或用户提供候选源、观测日期、beam、DM 和 burst TOA 秒数后，AFTER 负责完成裁切、定标、爆发区域复核、能量和偏振分析、结果表导出，以及观测面板汇总。
...
- **自包含观测面板**：从 `burst_results.csv` 生成 HTML 面板，汇总图表、burst 目录、诊断图嵌入和可靠 RM 提示。
...
flowchart LR
    ...
    G --> H["burst_dashboard.py<br/>HTML 面板"]
```

完整流程：

```text
切数据 -> 定标 -> 爆发探测 -> 人工检查标记 -> 能量/偏振分析 -> 出表 -> 生成面板
```
...
| `burst_dashboard.py` | 根据 `burst_results.csv` 和 analysis 诊断图生成自包含 HTML 观测面板。 |
...
`burst_dashboard.py` 读取 `burst_results.csv`，生成一个可以直接在浏览器打开、也可以打印成 PDF 的自包含 HTML 面板。
...
面板顶部指标包括 burst 数、时间跨度、事件率、最高 S/N、DM 范围、宽度范围、可靠 RM 数量，以及 `FLUENCE x BW = sum(fluence * bandwidth_GHz)`，单位为 `Jy ms GHz`。这个量只是观测侧的 fluence-bandwidth 汇总，不是各向同性能量、接收能量，也不包含光度距离或红移因子。
...
- 诊断图和观测面板：`*.jpg`、`*.png`、`burst_dashboard.html`
